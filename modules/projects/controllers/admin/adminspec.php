...
class Adminspec extends Admin_Controller {
  function Adminspec(){
   	parent::Admin_Controller();
		   // Check for access permission
			...
			// Set breadcrumb
			$this->bep_site->set_crumb('Project Spec','projects/admin/adminspec');
	}
	...
	$data['page'] = $this->config->item('backendpro_template_admin') . "spec/admin_spec_home";
    ...
    function showspec($id=NULL){
        $data['title'] = "Project Spec";
        $data['header'] = 'Project Spec';
        // Set breadcrumb
	$this->bep_site->set_crumb('Show Spec','projects/admin/adminspec/showspec');
        ...
        // get spec
        $spec =  $this->MProjectspec->getProjectSpec($project_id);
        $data['specs']=$spec;
        ...
        if($this->input->post('spec_desc')){
            ...
            flashMsg('info','New Spec is created.');
            redirect('projects/admin/adminspec/showspec/'.$customer_id."/".$project_id,'refresh');
        }

        $data['page'] = $this->config->item('backendpro_template_admin') . "spec/admin_show_spec";
	$data['module'] = 'projects';
	$this->load->view($this->_container,$data);
    ...
    function update_spec($id=NULL){
        ...
        if ($this->input->post('spec_desc')){
            ...
            $data['title'] = "Update Spec";
            $data['header'] = 'Update Spec';
            $data['page'] = $this->config->item('backendpro_template_admin') . "spec/admin_spec_edit";
            
            // find spec_id
            $spec_id = $this->uri->segment(7);
            $records = $this->MProjectspec->getSigleSpec($spec_id);
            ...
            //$data['members'] = $this->user_model->getUsers();// not working at the moment
              ...
              //  if (!count($data['records'])){
                ...
                // Set breadcrumb
                $this->bep_site->set_crumb('Update Spec','projects/admin/adminspec/update_spec');
                $data['module'] = 'projects';
                $this->load->view($this->_container,$data);
        }
...
// Instead of using AjaxinserShoutBox we use IS_AJAX here
    ...
    function delete_spec($id = NULL){
            $customer_id = $this->uri->segment(5);
            $project_id = $this->uri->segment(6);
            $id = $this->uri->segment(7);
            $this->MProjectspec->deletespec($id);
            flashMsg('success','Spec deleted');
            redirect('projects/admin/adminspec/showspec/'.$customer_id."/".$project_id,'refresh');
    }
